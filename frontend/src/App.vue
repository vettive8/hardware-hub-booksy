    ...
    <section class="workspace">
      ...
      <div v-else-if="view === 'admin'" class="content admin-page">
        ...
        <section class="admin-grid">
          ...
          <form class="card form-card" @submit.prevent="addUser"><div class="card-head"><div><h3>Create account</h3><p>Accounts are admin-issued only</p></div><span class="step">02</span></div><div class="form-grid"><label>Full name<input v-model="userForm.name" required placeholder="Jamie Doe"></label><label>Work email<input v-model="userForm.email" required type="email" placeholder="j.doe@booksy.com"></label><label>Password<span class="password-field"><input v-model="userForm.password" required :type="showNewPassword ? 'text' : 'password'" minlength="8" placeholder="Minimum 8 characters"><button type="button" class="peek" :aria-pressed="showNewPassword" @click="showNewPassword = !showNewPassword">{{ showNewPassword ? 'Hide' : 'Show' }}</button></span></label><label>Role<select v-model="userForm.role"><option value="user">User</option><option value="admin">Admin</option></select></label></div><button class="button primary" :disabled="busy">Create account →</button></form>
        </section>
